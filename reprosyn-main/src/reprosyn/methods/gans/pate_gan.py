...
sys.path.append('reprosyn-main/src/reprosyn/methods/data_synthesiser/')
...
class PateGan(GenerativeModel):
    ...
    def fit(self, data, targets):
        ...
        assert isinstance(
            targets, self.datatype
        ), f"{self.__class__.__name__} expects {self.datatype} as input data but got {type(targets)}"
        ...
        nsamples = len(data)
        features_train = self._encode_data(data, data)
        targets_encoded = self._encode_data(data, targets)
        ...
        with tf.device(self.device_spec.to_string()):
            ...
            # targets_output = tf.nn.sigmoid(self.discriminator_out(targets_encoded))
            # print([x[0] for x in targets_output.eval(session=self.sess)])
            ...
            for _ in range(self.n_iters):
                ...
                for _ in range(self.num_teachers):
                    ...
                    labels_batch = np.concatenate(
                        (np.ones([self.batch_size,]), np.zeros([self.batch_size,])), 0
                    )
            ...
            targets_output = tf.nn.sigmoid(self.discriminator_out(targets_encoded))
            target_results = [x[0] for x in targets_output.eval(session=self.sess)]
            # targets_output = np.array([x[0] for x in self.discriminator_out(targets_encoded).eval(session=self.sess)])

        self.trained = True
        # return activate_3(targets_output)
    ...
    def _encode_data(self, data_for_domain, data):
        n_samples = len(data)
        ...
        for attr_name, cdict in self.metadata.items():
            ...
            domain_col_data = data_for_domain[attr_name].to_numpy()

            if data_type == FLOAT or data_type == INTEGER:
                # Normalise continuous data
                if self.infer_ranges:
                    col_max = max(domain_col_data)
                    col_min = min(domain_col_data)
        ...
        return np.float32(features_encoded)
